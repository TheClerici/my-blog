...
Here are all of the properties needed for the to do application to work correctly, as I made all the validations on Controller and Repository, we will always input data that won't compromise our program. The contructor only contains `id`, `text`, `dueDate` and `proprity`, as the other parameters will be setup later by the user or by me to get certain attributes. Then, the getters and setters are defined (Will only show one as an example but all the variables have them) along with the toString() Override.
...
public class ToDo {
    ...
    private Duration betweenDate;           //Extra variable to get seconds between creation and done dates
