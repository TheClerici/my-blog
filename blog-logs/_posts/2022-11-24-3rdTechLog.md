...
In this modal we have a form that let's you input the Name of the To Do, it's priority and/or due date. it has two buttons as well, one to cancel and another one to submit. When succesfully submitted, it returns the values inputed to App.js so it can POST the new To Do using axios.
...
In this modal there are only two buttons, one to cancel and another to delete all To Do's, when choosing delete all it returns the values inputed to App.js to call an axios DELETE.
