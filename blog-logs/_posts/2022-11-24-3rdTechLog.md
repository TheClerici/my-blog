...
When edit is pressed, it opens up a modal that works exactly as the add To Do modal.\
When delete is pressed, it returns the id value of that To Do to App.js with props and calls an axios DELETE for that To Do.
...
Here, we receive through an axios GET the metrics and store them in a variable, then, with props, we send them to the stats component and showcase them.
